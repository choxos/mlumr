...
- **Integration refit.** Both models refitted with `n_int = 2048` against the base 512, same priors. Every refit uses its own seed (2026 plus the scenario number), so its Monte Carlo error is independent of the base fit's and the MCSE of a difference between rows is the square root of the sum of the two squared MCSEs.
