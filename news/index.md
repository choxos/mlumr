...
- Breaking: for `family = "normal"` the default priors are on the
  outcome’s scale. Under the identity link the intercepts and
  coefficients are in the outcome’s units, and the residual SD is under
  either link, so the fixed `normal(0, 10)` intercept and half-normal(0,
  2.5) residual-SD defaults were informative for an outcome such as a 0
  to 100 score or a blood pressure in mmHg. They pulled each intercept
  toward zero, the comparator’s (resting on one aggregate mean) the
  most, which moved the mean difference, and they understated the
  residual SD, which narrowed the intervals. With `sd(y)` the IPD
  outcome SD, the defaults are now `normal(0, 10 * sd(y))` for the
  ...
  `sd(y)` as well as dividing it by `sd(x)` under the identity link.
  Priors written out in full are used as given.
  [`prior_summary()`](https://choxos.github.io/mlumr/reference/prior_summary.md)
  prints the scales the model used, and
  [`prior_sensitivity()`](https://choxos.github.io/mlumr/reference/prior_sensitivity.md)
  sweeps a default or autoscaled `prior_beta` in the same outcome-SD
  units. Refit normal-family analyses that relied on the defaults or on
  `autoscale`.
...
- Fix: With the cmdstanr engine, `fit$summary$se_mean` is the Monte
  Carlo standard error of the posterior mean from
  [`posterior::mcse_mean()`](https://mc-stan.org/posterior/reference/mcse_mean.html).
  It was the posterior SD over the square root of the bulk ESS, which is
  computed on rank-normalized draws and misstates the error for skewed
  quantities such as a ratio.
- Fix: [`mlumr()`](https://choxos.github.io/mlumr/reference/mlumr.md)
