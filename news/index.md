...
- **A comparator curve whose event rows outnumber the rank of the
  ...
  scope, so a shared slope is reported instead of netted. Only from
  order TWO, though: in `(mu_index, mu_comparator, beta)` an index
  constraint is `(1, 0, x)` while every comparator constraint is
  `(0, 1, z)`, so no combination of comparator rows reaches a nonzero
  ...
  slope direction and can lie in the comparator’s span. It also needs
  somewhere to lie: a matched design of rank 1 is one row,
  ...
  exponentially suppress was refused. The eventless path now carries
  that design, and the pinned-ridge report asks whether the index pins
  the slope rather than whether its EVENT design fits exactly.
