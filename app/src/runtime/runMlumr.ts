...
export async function runMlumrModel(
  ...
  if (numChains === 1) {
    const chain = await runChain(modelName, baseConfig(1), callbacks);
    return mergeChains([chain], samplingOptions, seed, (performance.now() - start) / 1000);
  }

  // Multiple chains: one worker each, bounded by core count. Only chain 1
  // streams progress so the live log is not interleaved.
