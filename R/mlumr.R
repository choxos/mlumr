...
#' count, and time-to-event (survival) outcomes.
#'
...
#' @param seed Random seed for reproducibility. If `NULL` (default), a seed is
#'   drawn from R's RNG when set.seed() has been called, otherwise the fixed
#'   default 2026 is used (with a warning). The seed actually used is reported in
#'   the fitting messages.
...
mlumr <- function(data,
  ...
  # prior_beta_comparator: NULL means "same as prior_beta"; otherwise validate
  # the same way as prior_beta. Only consumed by the relaxed models; warn (do
  # not error) if supplied for SPFA so users learn the arg is a no-op there.
      ...
      if (agd_rank < n_cov_check + 1L) {
        warning(sprintf(
          paste0("Relaxed model with %d AgD row(s), aggregate design rank %d, ",
                 ...
                 "`prior_beta_comparator` (see ?mlumr) or use model = \"spfa\"."),
...
.mlumr_build_stan_data <- function(data, family, link_info, prior_intercept,
  ...
  if (family == "binomial") {
    ...
    if (nrow(agd_data) > 1L &&
        (is.null(agd_data$.n) || any(!is.finite(agd_data$.n)) ||
           any(agd_data$.n <= 0))) {
...
.agd_center_weights <- function(stan_data, family, n_agd_rows) {
  ...
  # pseudo-individuals, so falling through to the equal-split branch below would
  # give every AgD row the same weight regardless of how many pseudo-individuals
  # it actually contributes. That is the tabulation dependence this function
  # exists to remove: two comparator arms of 300 and 60 would be centered as if
  # they were 180 each. `agd_arm` maps each pseudo-individual to its row, so
  # tabulating it recovers the true per-row population.
  if (is.null(w) && identical(family, "survival")) {
    ...
    w <- if (!is.null(arm) && n_agd_rows >= 1L) {
      ...
      tabulate(arm_int, nbins = n_agd_rows)
    } else {
      stan_data$n_agd
    }
  }
...
.resolve_mlumr_seed <- function(seed) {
  ...
  if (exists(".Random.seed", envir = globalenv(), inherits = FALSE)) {
    # The user has set R's RNG state (e.g. set.seed()); derive the Stan seed from
    # it so the fit is reproducible together with the rest of their script.
    return(list(value = sample.int(.Machine$integer.max, 1L), source = "set.seed"))
  }
  warning("No `seed` supplied and set.seed() has not been called; using the ",
          "default seed 2026 so the fit is reproducible. Call set.seed() before ",
          "mlumr(), or pass `seed = `, to control it.", call. = FALSE)
...
.mlumr_log_fit_start <- function(model_name, family, link, stan_data,
  ...
  if (family == "binomial") {
    ...
    mlumr_message(sprintf("  AgD: %d arm(s), %d pseudo-individuals, %d events",
                          stan_data$n_agd_rows, stan_data$n_agd,
                          sum(stan_data$agd_status == 1L)),
                  verbose = verbose)
  }
  ...
  seed_note <- switch(seed_info$source,
                      "default" = " (default)",
                      "set.seed" = " (from set.seed())",
                      "")
