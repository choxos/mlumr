...
summary.mlumr_fit <- function(object, ...) {
  ...
  if (family == "binomial") {
    ...
    delta_idx <- grep("^delta_(index|comparator)$", object$summary$variable)
    if (length(delta_idx) > 0) {
      # What this scalar IS comes from the same helper marginal_effects() and
      # prior_sensitivity() use, so the three surfaces cannot disagree. Deciding
      # it here independently is what previously let a relaxed AFT fit with one
      # shared shape print "Log Time Ratios": the treatment-specific
      # coefficients mean the covariate term does not cancel, so the scalar is a
      # linear-predictor contrast rather than a population time ratio.
      lab <- .surv_scalar_label(object, log_scale = TRUE)
      hdr <- switch(lab$label,
        LOG_HR = if (is.finite(lab$at_time) && lab$at_time > 0) {
          sprintf("  Log Hazard Ratios (marginal, at t = %s):\n",
                  format(lab$at_time, digits = 4L))
        } else {
          "  Log Hazard Ratios (marginal, t -> 0 limit):\n"
        },
        LOG_TR = "  Log Time Ratios (population acceleration factor):\n",
        DELTA_ETA = paste0("  Log linear-predictor contrasts (eta_index - eta_comparator).\n",
                           "   These are NOT population log time ratios: the covariate term does\n",
                           "   not cancel here, so the contrast varies with the covariates.\n"),
        "  Treatment effects:\n"
      )
      is_ph <- identical(lab$label, "LOG_HR")
      cat(hdr)
      print(object$summary[delta_idx, c("variable", "mean", "sd", "2.5%", "97.5%")],
            row.names = FALSE)
      if (is_ph && differs) {
        cat("   (marginal HRs are non-collapsible and vary with time; pass",
            "\n    `at_time` to marginal_effects() to choose the evaluation time)\n")
      }
    }
    rmstd_idx <- grep("^rmst_diff_", object$summary$variable)
    if (length(rmstd_idx) > 0) {
      # An RMST difference is defined only together with its restriction time,
      # and when no horizon was requested mlumr() defaults a study-stratified
      # flexible baseline to the common support rather than the pooled maximum.
      # Print the horizon actually integrated to, read off the fitted grid.
      g <- object$stan_data$rmst_grid_times
      if (is.null(g)) {
        cat("  RMST Differences:\n")
      } else {
        cat(sprintf("  RMST Differences (horizon tau = %.4g):\n", max(g)))
      }
      print(object$summary[rmstd_idx, c("variable", "mean", "sd", "2.5%", "97.5%")],
            row.names = FALSE)
    }
  }
