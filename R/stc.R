...
.stc_poisson <- function(data, fit, ipd, agd, newdata, link_resolved,
  ...
  rate_hat_A_se <- if (var_lrr_A == 0) 0 else
    exp(log_rate_hat_A + 0.5 * log(var_lrr_A))

  # Rate difference on the natural (events per unit exposure) scale, the count
  # analogue of the binomial risk difference: the standardized index rate minus
  # the observed comparator rate. The delta-method variance of the standardized
  # rate and the comparator's Poisson variance are independent and add.
  rate_B_var <- rate_B / exposure_B
  rd <- if (rate_B > 0) {
    .exp_difference_logs(log_rate_hat_A, log(rate_B))
  } else {
    rate_hat_A
  }
  rd_se <- .stc_hypot(rate_hat_A_se, sqrt(rate_B_var))
  ...
  list(
    ...
    rd_lower = rd - z * rd_se,
    rd_upper = rd + z * rd_se,
    rate_index_comparator = rate_hat_A,
    rate_hat_index = rate_hat_A,
    rate_hat_index_se = rate_hat_A_se,
    rate_comparator = rate_B,
    rate_comparator_se = sqrt(rate_B_var),
