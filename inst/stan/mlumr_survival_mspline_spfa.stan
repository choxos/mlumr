// ML-UMR: flexible-baseline survival with the Shared Prognostic Factor
// Assumption (SPFA). Baseline hazard is an M-spline (piecewise exponential is
// the degree-0 special case); proportional hazards on exp(eta). Unanchored
// comparison of single-arm trials.
//
//   h(t|x) = h0(t) exp(eta),  h0(t) = sum_j scoef_j M_j(t)
...
data {
  ...
  matrix[n_ipd, n_scoef] b_ipd;               // M-basis at event/censoring time
  matrix[n_ipd, n_scoef] ib_ipd;              // I-basis (integrated) at time (upper)
  matrix[n_ipd, n_scoef] ib_ipd_start;        // I-basis at interval lower bound (else 0)
  matrix[n_ipd, n_scoef] ib_ipd_delay;        // I-basis at delayed-entry time (else 0)
  ...
  // HTA prediction grid (basis evaluated in R)
  int<lower=1> n_pred_times;
  vector<lower=0>[n_pred_times] pred_times;
  matrix[n_pred_times, n_scoef] pred_basis;   // M-basis at pred_times (hazard)
  matrix[n_pred_times, n_scoef] pred_ibasis;  // I-basis at pred_times (survival)
...
#include include/priors_hyperparameters.stan
  ...
  // RW1 anchor + weights for the log-ratio spline coefficients.
  matrix[n_scoef - 1, n_strata] lscoef_prior_mean;
  matrix<lower=0>[n_scoef - 1, n_strata] lscoef_weights;

  // Smoothing SD prior on the RW1 innovations.
...
transformed parameters {
  ...
  // IPD log-likelihood contributions (status-aware, delayed entry as no-op
  // when ib_ipd_delay is the zero row).
...
model {
  ...
  // Comparator pseudo-IPD likelihood, integrated over comparator covariates.
  {
...
generated quantities {
  // Proportional-hazards baseline: treatment effect is a log hazard ratio.
  ...
  {
    vector[n_ipd] eta_idx_ip = mu_index + X_ipd * beta;
    vector[n_ipd] eta_cmp_ip = mu_comparator + X_ipd * beta;
    vector[n_int_all] eta_idx_cp = mu_index + X_int_all * beta;
    vector[n_int_all] eta_cmp_cp = mu_comparator + X_int_all * beta;
