...
- **Key points board.** The board on the right, labeled Key points (it was labeled Notes), holds the narration's notes: a statement or formula per chapter, shown as the narration reaches it. It now follows the chapter on screen as well: while the learner explores another chapter, the board shows the points the narration has shown by the end of that chapter, cloned from the player's own items, and Return to narration brings the narration's board back. Before this change a learner who paused at the start and moved through the chapters saw the first chapter's question on the board throughout.
...
- `browser-qa.mjs --no-runtime` in Playwright's bundled Chromium (`QA_BROWSER_CHANNEL=`) on the build with the key points board change: 0 page errors, 0 console errors, 0 failed requests; with the narration paused in the survival chapter, choosing the target chapter from the menu showed a board with exactly the item the tracks mark shown by the end of that chapter (the risk difference definition) and hid the player's own items, and Return to narration removed the copy and showed the player's board again; every other check passed again.
...
- **Screens and contrast.** Light muted text, reference lines and the review color were darkened to pass WCAG contrast. On phones the key points board is hidden, the chapter list stays on screen, the header fits at 320 pixels, charts keep readable labels and scroll inside their cards, and portrait orientation is allowed.
