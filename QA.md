...
- `lesson.sh test`: strict TypeScript passed for the scene and, with the WebWorker library, for the Stan worker. **66 unit tests** passed in 6 files:
  ...
  The chart, code cell and runner tests fail when run against the modules they replaced, and the four restart and cancel tests fail against the modules of the commit before each was added.
