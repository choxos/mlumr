...
uses. That example fits it on four studies with treatment classes sharing their
interactions; here SEC_300 is seen only in aggregate data and shares with
nothing, so its interactions are not identified (the table's note below says
why). Both are reported so the difference is visible rather than asserted. Note
also that once covariates
...
than two log odds ratios, and that difference is set by interaction slopes the
data never informed, so it says nothing about how the contrast really varies
between the populations. A large difference is not by itself a sign of weak
identification, since strong, well-estimated effect modification would produce
one too; the width of the intervals is, and here it is the width that rules
these rows out. They are included so that both their width and their
...
Nothing about the data changed between those two rows: a single aggregate row
constrains the comparator coefficients only through one combination with the
comparator intercept, and the index-population estimand extrapolates them
across the IPD covariate distribution. A reader shown
