...
so $\mu_k$ and $\beta_k$ live on the **log-count** scale (here there is one row
...
covariates <- c("age", "log_cfu")   # baseline bacterial load is the key prognostic
...
The whole point of population adjustment is that the two trial populations differ
...
load (`log_cfu`) is the key prognostic factor:
...
#> Marginals -- max relative difference: 0.0122
...
Both return estimates on the **log-count** (link) scale, so we exponentiate them
...
`log(exposure)` as the offset, so coefficients are on the log-count scale. We fit
...
#>       mu_index 1.984391 0.03753292 1.912305 2.057652 1.000861
#>  mu_comparator 2.193849 0.03531095 2.122470 2.263489 1.002208
#> 
#> Regression Coefficients:
#>       variable          mean         sd        2.5%       97.5%     Rhat
#>      beta[age] -0.1160702769 0.05087258 -0.21649153 -0.01789344 1.000664
#>  beta[log_cfu]  0.0003751122 0.01286532 -0.02407218  0.02556729 1.001259
#> 
#> Marginal Treatment Effects:
#>   Rate Ratios:
#>          variable     mean         sd      2.5%    97.5%
#>       delta_index 0.812105 0.04199963 0.7335938 0.897529
#>  delta_comparator 0.812105 0.04199963 0.7335938 0.897529
```
...
is Poisson; if the IPD shows clear overdispersion (compare `var(dmft)` against
`mean(dmft)`), treat the intervals as a lower bound on uncertainty, add
prognostic covariates that absorb the extra variation, and lean on the naive and
STC benchmarks.
...
|:----------------|:------|:----------|--------:|---------:|---------:|---------:|--------:|
|delta_index      |RR     |Index      | 0.812105| 0.0419996| 0.7335938| 0.8106831| 0.897529|
|delta_comparator |RR     |Comparator | 0.812105| 0.0419996| 0.7335938| 0.8106831| 0.897529|



A forest plot makes the value of adjustment visible. It uses a log axis (null
at 1); the frequentist `naive`/`stc` estimates are on the log-count scale, so we
...
|:-------|:----------------|:------|:----------|---------:|---------:|---------:|---------:|---------:|
|SPFA    |delta_index      |RR     |Index      | 0.8121050| 0.0419996| 0.7335938| 0.8106831| 0.8975290|
|SPFA    |delta_comparator |RR     |Comparator | 0.8121050| 0.0419996| 0.7335938| 0.8106831| 0.8975290|
|Relaxed |delta_index      |RR     |Index      | 0.7018503| 0.2043127| 0.1928145| 0.7542851| 0.9799609|
|Relaxed |delta_comparator |RR     |Comparator | 0.8121465| 0.0423406| 0.7319699| 0.8103883| 0.8973908|
...
|:---------|:----------|--------:|---------:|--------:|--------:|--------:|
|SDF       |Index      | 7.254688| 0.2687005| 6.740638| 7.254038| 7.797880|
|NSF       |Index      | 8.945033| 0.3296163| 8.307245| 8.933490| 9.606851|
|SDF       |Comparator | 7.367297| 0.2876052| 6.819380| 7.362904| 7.931766|
|NSF       |Comparator | 9.082315| 0.3099364| 8.476446| 9.078960| 9.710237|
...
|-------:|:------|--------:|---------:|---------:|---------:|--------:|
|       1|RR     | 0.812105| 0.0419996| 0.7335938| 0.8106831| 0.897529|
|       2|RR     | 0.812105| 0.0419996| 0.7335938| 0.8106831| 0.897529|
|       3|RR     | 0.812105| 0.0419996| 0.7335938| 0.8106831| 0.897529|
...
#>     SPFA      -0.1     0.1    0.99 |elpd_diff| < 4 1 k_psis > 0.7
#> 
...
#>  Relaxed 518.82 4.13      0.00
#>     SPFA 518.98 4.25      0.16
#> 
