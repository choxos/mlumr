...
different units. For `family = "normal"` the intercepts, the identity-link
coefficients and the residual SD are in the outcome's own units, so there the
defaults and autoscaled scales are also multiplied by the IPD outcome SD (see
`vignette("continuous-outcomes")`). Inspect the priors actually passed to
Stan, including autoscaled scales, with `prior_summary()`:
...
For convergence diagnostics on the raw draws, `bayesplot` needs to know which
chain each draw came from. `fit$draws` is a flat data frame with the chains
already stacked, so passing it directly draws one merged line per parameter.
That looks like a healthy trace whatever the chains did, because the thing a
trace plot exists to show, whether the chains agree with each other, has been
averaged away before plotting. The per-draw chain labels are kept beside the
draws in `fit$chain_ids`, so reshape into the iteration by chain by parameter
array `bayesplot` expects:
...
Each chain is drawn separately, which is what makes the plot readable as a
diagnostic: chains that settled in different places show up as separated bands.
`check_diagnostics()` answers the same question numerically through split-Rhat
and effective sample size, and the two should agree.
