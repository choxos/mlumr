...
`naive()` compares the two arms as reported and uses no covariates at all,
so it needs no integration points and can be called immediately after
`combine_data()`.

`stc()` does need them, except under an identity link. It standardizes the
index outcome model over the comparator population, and on a nonlinear link
the mean of the predictions is not the prediction at the mean: substituting
the aggregate covariate means for the distribution is biased by Jensen's
inequality, in a direction that depends on the curvature. `stc()` refuses
that substitution rather than returning a number that looks usable.

```{r, eval = TRUE}
dat_no_int <- combine_data(ipd, agd)

# naive() needs nothing beyond combine_data()
naive_result <- naive(dat_no_int)

# stc() on a logit link needs the comparator distribution
stc_result <- stc(dat)
```
