...
hazard ratio at one instant, and the `at_time` column says which: the
$t \rightarrow 0$ limit for an unstratified fit, and otherwise the first
prediction time, which is positive and set by `pred_times`. Prefer
`predict(type = "loghr")` for the time-varying contrast, or the collapsible
RMST estimands (`effect = "rmstd"` / `"rmstr"`), whose horizon is reported
beside them. **Caveat (unanchored two-study setting).**
