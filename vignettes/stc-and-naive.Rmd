...
```{r, eval = TRUE, purl = FALSE}
# STC marginalizes over the comparator covariate distribution, so it needs
# the integration points that describe it.
...
- **At the AgD covariate means only** (a single point), a nonlinear model is
  biased by Jensen's inequality: the mean of the predictions is not the
  prediction at the mean. `stc()` therefore refuses this case rather than
  returning a number that looks usable, unless the link is the identity, where
  the two coincide.
- **With integration points**, STC marginalizes over the full covariate
  distribution, which is the estimand it is meant to report.
...
# The identity link is the one case where the mean of the predictions equals
# the prediction at the mean, so no integration points are required here.
naive(dat_normal)
stc(dat_normal)
```
...
# The log link is nonlinear, so STC needs the comparator distribution rather
# than its means.
dat_poisson <- add_integration(dat_poisson, n_int = 64,
                               age_cat = distr(qbern, prob = age_cat_mean))
naive(dat_poisson)
stc(dat_poisson)
```
