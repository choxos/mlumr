...
`rmst*`). For survival, RMST-based rows also carry a `horizon` column
(the raw-draw frame, a `horizon` attribute) giving the restriction time
the integral runs to. RMST at different horizons is a different
estimand, so results are only comparable across fits when this value
matches.
