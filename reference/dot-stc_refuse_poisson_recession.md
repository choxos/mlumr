...
anyway, when the deviance stops changing: 80 zero counts on a
nonconstant covariate stop after 25 iterations at an intercept near
-27.3 with a standard error near 57,500, and 40 zeros beside 40 positive
counts on a binary covariate stop at a slope near 21.2. Every number
there describes where the iteration stopped, not the data, and the
fitted-value screen for the binomial case does not apply: a rate can
legitimately be small.
