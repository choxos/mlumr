...
- ref_line:
  ...
  column when the label is one the package produces (`1` for a ratio
  measure, `0` for a difference); otherwise `1` when `log_x = TRUE` and
  `0` otherwise. Pass it explicitly for a measure this does not name.
