...
interface NativeRecord {
  ...
  package: { version: string; engine: string; r: string; cmdstan?: string | null; loaded_from?: string; commit?: string | null; dirty?: boolean | null };
...
<p>From <code>Rscript workflow.R --fit --sensitivity --record</code> at mlumr ${esc(native.package.version)}${native.package.commit ? ` (commit ${esc(native.package.commit.slice(0, 7))}${native.package.dirty ? ', tree with local changes' : ', clean tree'})` : ''} with ${esc(native.package.engine)}${native.package.cmdstan ? ` (CmdStan ${esc(native.package.cmdstan)})` : ''}, run ${esc(native.run)}. The base, integration and comparator prior rows are the risk difference, A minus B, in the same prespecified 400-row target, re-extracted from each refit. The transport rows do not refit: they evaluate the two base fits in the three 400-row targets named in the Target column. The true value behind the simulated data in the prespecified target is ${cell(native.truth.target_rd, 3)}. MCSE is the Monte Carlo standard error of the posterior mean.</p>
