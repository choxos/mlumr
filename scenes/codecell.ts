...
function benchmarkView(name: string, meaning: string, b: Benchmark) {
  const value = usable(b) ? `${num(b.estimate!)} (${Math.round(100 * (b.conf_level ?? .95))}% confidence interval ${num(b.lower!)} to ${num(b.upper!)})` : `not usable${b.error ? `: ${b.error}` : ''}`;
