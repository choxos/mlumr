...
test_that("the WEAK flag uses the same floor as the rank screen", {
  # The `flagged` screen and `.profile_rank()` compare the same spread against
  # the same floor with the same slack, so a design inside the slack window
  # cannot be counted as a direction by one and called WEAK by the other.
...
test_that("the two identification screens agree on an undecomposable design", {
  # Both screens are handed the same matrix by `check_identification()`, so a
  # mean matrix carrying NA must fail closed in both rather than abort in one.
