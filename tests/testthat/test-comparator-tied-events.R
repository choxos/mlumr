...
test_that("an unmatched node past the censoring time certifies the ridge", {
  ...
  expect_equal(as.vector(attr(idx, "index_slope")), 0)
})
...
test_that("a rank-dropped solve does not supply a pinned slope", {
  ...
  expect_equal(as.vector(ps(cbind(1, c(-1, 1)), c(-1, 1))), 1)
  expect_equal(as.vector(ps(cbind(1, c(-1, 1)), c(0, 0))), 0)
...
test_that("an amplified slope error does not certify a ridge", {
  ps <- mlumr:::.pinned_slope
  # A residual that looks like rounding on the INDEX rows is not small
  # enough to certify anything about the comparator's. The slope error it
  # implies is `norm(r) / sigma_min`, and a node at distance `d` from the
  # matched one turns that into `d * norm(r) / sigma_min` in the predictor.
  fit_at <- function(z) {
    d <- cbind(1, z)
    ps(d, as.vector(d %*% c(0.3, 2)))
  }
  well <- fit_at(c(-2, -1, 0, 1, 2))
  expect_equal(as.vector(well), 2)
  expect_lt(attr(well, "err_factor"), 1e-15)
  # The same slope on a design whose spread is 1e-9 solves to rounding and
  # still carries a factor nine orders of magnitude larger, because the
  # residual is divided by a tiny smallest singular value.
  # The same slope on a design whose spread is 1e-9 is returned WRONG in the
  # eighth digit while its residual still looks like rounding, which is the
  # whole finding: the residual is measured on the index rows and the error
  # is felt at the comparator's.
  ill <- fit_at(c(0, 1e-10, 3e-10, 7e-10, 1.1e-9))
  expect_gt(abs(as.vector(ill) - 2), 1e-8)
  expect_lt(abs(as.vector(ill) - 2), 1e-6)
  # And the factor covers that error rather than merely noticing it.
  expect_gt(attr(ill, "err_factor"), abs(as.vector(ill) - 2))
  expect_gt(attr(ill, "err_factor") / attr(well, "err_factor"), 1e6)
  # A residual above rounding is not a solution at all and answers nothing.
  expect_null(fit_at(1e8 + c(0, 1, 2, 3, 4)))
  # And that factor is what the certificate has to clear. The same comparator
  # that refuses on a well-conditioned `beta = 1` is reported when the slope
  # carries an error bound big enough to move a node across the boundary.
  d <- .comp_stub(c(1, 1, 2), c(1L, 1L, 0L),
                  ipd_time = c(1, 1), ipd_x = c(-1, 1), n_int = 8)
  tilt <- ps(cbind(1, c(-1, 1)), c(-1, 1))
  expect_match(msg(d, aux_by = "none", model = "spfa", index_exact = TRUE,
                   index_design = cbind(1, c(-1, 1)), index_slope = tilt),
               "is therefore improper")
  blunt <- structure(1, err_factor = 1)
  w <- tryCatch(check(d, aux_by = "none", model = "spfa", index_exact = TRUE,
                      index_design = cbind(1, c(-1, 1)),
                      index_slope = blunt),
                warning = conditionMessage)
  expect_match(w, "the shared `beta`")
  expect_no_match(w, "is therefore improper")
  # A slope with no bound at all certifies nothing.
  w2 <- tryCatch(check(d, aux_by = "none", model = "spfa", index_exact = TRUE,
                       index_design = cbind(1, c(-1, 1)), index_slope = 1),
                 warning = conditionMessage)
  expect_match(w2, "the shared `beta`")
})
